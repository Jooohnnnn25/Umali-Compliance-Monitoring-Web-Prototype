  ...
  const renderItem = ({ item }) => (
    <View style={styles.row}>
      ...
      <View
        style={[
          styles.cell,
          { flex: 1.8, flexDirection: "row", alignItems: "center" },
        ]}
      >
        <Picker
          selectedValue={viewSelections[item.id] || "application"}
          style={{ height: 40, flex: 1 }}
          onValueChange={(val) =>
            setViewSelections((prev) => ({ ...prev, [item.id]: val }))
          }
          mode="dropdown"
        >
          <Picker.Item label="View Application" value="application" />
          <Picker.Item label="View Compliance Progress" value="progress" />
        </Picker>

        <TouchableOpacity
          style={styles.goButtonSmall}
          onPress={() =>
            router.push(`/${viewSelections[item.id] || "application"}`)
          }
        >
          <Text style={styles.goButtonSmallText}>Go</Text>
        </TouchableOpacity>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.row, styles.headerRow]}>
        ...
        <Text style={[styles.headerCell, { flex: 1.8 }]}>View</Text>
